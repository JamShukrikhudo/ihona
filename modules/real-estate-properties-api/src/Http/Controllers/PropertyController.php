...
final class PropertyController
{
    ...
    public function store(Request $request, CreateProperty $create): JsonResponse
    {
        $validated = $request->validate([
            ...
            'postal_code' => ['sometimes', 'nullable', 'string', 'max:20'],
            'country' => ['sometimes', 'nullable', 'string', 'size:2'],
            'region_id' => ['sometimes', 'nullable', 'integer', Rule::exists('real_estate_regions', 'id')],
            'city_id' => ['sometimes', 'nullable', 'integer', Rule::exists('real_estate_cities', 'id')],
            'district_id' => ['sometimes', 'nullable', 'integer', Rule::exists('real_estate_districts', 'id')],
            ...
            'mountain_view' => ['sometimes', 'nullable', Rule::in(['pamir', 'fan', 'hissar', 'other'])],
            'altitude' => ['sometimes', 'nullable', 'integer', 'min:0'],
            'water_source' => ['sometimes', 'nullable', Rule::in(['well', 'river', 'spring', 'other'])],
    ...
    public function walkability(Request $request, Property $property, FetchWalkabilityScores $fetch, UpdateProperty $update): JsonResponse
    {
        ...
        $updatedProperty = $update->handle($property->team_id, $user->getAuthIdentifier(), $property->getKey(), $scores);
