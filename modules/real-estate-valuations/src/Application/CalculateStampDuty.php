...
final class CalculateStampDuty {
    public function handle(float $purchasePrice, string $buyerType = 'home_mover'): array {
        if ($purchasePrice < 0 || ! in_array($buyerType, ['home_mover', 'first_time_buyer', 'additional_property'], true)) throw ValidationException::withMessages(['purchase_price' => 'Purchase price and buyer type must be valid.']);
