...
final class AnalyzePropertyInvestment
{
    ...
    {
        $comparables = $options['market_data'] ?? [];
        $matching = array_values(array_filter($comparables, fn ($row) => (string) ($row['property_type'] ?? '') === (string) $property->property_type));
        $average = collect($matching)->avg('avg_price');
        $price = (float) $property->price;
        $roi = $average !== null && $price > 0 ? round(($average - $price) / $price * 100, 2) : 3.0;
        $position = $average === null ? 'average' : ($average > $price ? 'good' : ($average < $price ? 'poor' : 'average'));
