...
@if ($tone)
    ...
    {{-- The chevron goes through the attribute bag, not a second style
         attribute: a caller passing their own style would leave two on the tag
         and the browser would silently drop one of them. --}}
    <span {{ $attributes
              ->class([
                  'inline-flex min-w-8 items-center justify-center py-[3px] pl-2 pr-3',
